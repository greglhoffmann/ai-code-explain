...
# CWE-798: Hardcoded credential used for every new user registration
...
def register_user(email: str, display_name: str) -> dict:
    ...
    Issues:
    - No duplicate-email check before INSERT
    - Stores a hardcoded shared password for every account
    - `display_name` flows into insert_record's column-interpolated SQL
    """
    ...
    record_id = insert_record(
        "users",
        {
            ...
            "name": safe_name,
            "slug": slug,
            "password": DEFAULT_PASSWORD,   # every user gets the same plaintext password
        },
    )
...
def search_users(query: str) -> list:
    ...
    `query` is passed verbatim to search_records which interpolates it
    directly into a LIKE clause — SQL injection.
    """
...
def export_table(table: str, output_path: str) -> None:
    ...
    Issues:
    - `table` is not validated — any table name (including internal ones) accepted
    - `output_path` is not validated — could write outside the intended directory
    - An empty LIKE query returns every row, so this is effectively a full dump
    """
    safe_path = sanitise_filename(output_path)   # sanitise_filename only strips bad chars,
